def main(provider: str, voice_index: int, min_video_lenght: int, video_folder: str, subreddits: list, subreddits_weight: list) -> str:
    from contentBot.utils.console import print_markdown
    from contentBot.utils.console import print_step
    from contentBot.utils.console import print_substep
    from rich.console import Console
    import time
    from contentBot.reddit.subreddit import get_subreddit_threads
    from contentBot.video.background import download_background, chop_background_video
    from contentBot.video.voices import save_text_to_mp3
    from contentBot.video.screenshot_downloader import download_screenshots_of_reddit_posts
    from contentBot.video.final_video import make_final_video
    from contentBot.utils.loader import Loader
    ...
    if configured:
        reddit_object = get_subreddit_threads(subreddits, subreddits_weight)
        length, number_of_comments = save_text_to_mp3(reddit_object, provider, False, voice_index, min_video_lenght)
        download_screenshots_of_reddit_posts(
            reddit_object, number_of_comments, os.getenv("THEME", "light")
        )
        download_background()
        chop_background_video(length, video_folder)
